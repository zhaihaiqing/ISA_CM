...
#define UART1_RBUF_SIZE   4096		//要求：2 的整次幂
...
typedef struct uart1_rbuf_st
{
	unsigned int 	in;							//输入
	unsigned int 	out;						//输出
	//unsigned char 	tx_flag;					//发送一帧标志
	unsigned char 	rx_flag;					//接收一帧标志
	//unsigned int  	tx_len;						//串口发送长度
	unsigned int  	rx_len;						//串口接收长度
	//unsigned char  	tx_buf [UART1_RBUF_SIZE];	//串口发送缓冲区
	unsigned char  	rx_buf [UART1_RBUF_SIZE];	//串口接收缓冲区
}UART1_RBUF_ST;
